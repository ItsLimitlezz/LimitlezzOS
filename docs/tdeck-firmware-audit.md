...
| P1 | Delivery status | Newly sent DM packet IDs and `SENDING/DELIVERED/FAILED` state persist in message-log records; immediate backend failures, automatic retry attempts, retry counts, and failure reasons now surface in UI/serial diagnostics. | The ACK/retransmit path still needs stock-device and failure-mode validation. | Add ACK/retransmit tests and hardware interop coverage. |
...
| P2 | GPS/position/telemetry | Meshtastic POSITION and TELEMETRY packets now decode into node detail and persistence; GPS toggle and sample map/weather app data still exist separately. | Map, weather, telemetry, and emergency features still need app-facing consumers plus local GPS/platform hooks. | Validate with stock devices, then wire map/weather/emergency consumers to the decoded node data. |
...
3. Finish Meshtastic release polish before expanding scope: run the hardware dogfood checklist, complete the responsiveness pass, and validate ACK/retransmit behavior against stock devices.
