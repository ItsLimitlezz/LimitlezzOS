...
- Implement roadmap items 0.42 through 0.45:
  ...
  - responsiveness pass for Settings, chat log, keyboard input, and long lists. In progress: conversation typing/backspace now refreshes the compose label in place instead of rebuilding the full chat log; Contacts now uses the virtualized list helper and virtual-list touch scroll stays bounded; Settings brightness left/right updates the slider in place instead of rebuilding the whole screen. Still open: chat-log scroll and hardware latency regression proof.
