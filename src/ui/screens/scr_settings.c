...
static const srow_t SROWS[] = {   /* unsized: never drops a row when one is added */
    ...
    { "App source",       LZ_I_STOREFRONT, ROW_VALUE  },  /* f=5  */
    ...
    { "Clock format",     LZ_I_SCHEDULE,   ROW_VALUE  },  /* f=12 */
    { "Power saving",     LZ_I_BOLT,       ROW_TOGGLE },  /* f=13 */
    { "System & battery", LZ_I_MONITORING, ROW_NAV    },  /* f=14 */
    { "Calibrate touch",  LZ_I_LOCATION,   ROW_NAV    },  /* f=15 */
    { "Developer Mode",   LZ_I_TERMINAL,   ROW_TOGGLE },  /* f=16 */
};
#define SETTINGS_FOCUS_COUNT 17   /* 2 network rows + 15 SROWS */
...
static void settings_activate(int f)
{
    ...
    switch(f) {
        ...
        case 5: cycle(&S.settings.app_source, LZ_APP_SOURCE_COUNT); break;
        ...
        case 12: S.settings.clock24 = !S.settings.clock24; lz_svc_set_clock24(S.settings.clock24); break;
        case 13: S.settings.save = !S.settings.save; break;
        case 14: lz_go(LZ_V_SYSTEM); return;
        case 15: S.cal_step = 0; lz_go(LZ_V_TOUCHCAL); return;
        case 16: S.settings.developer = !S.settings.developer; break;
...
void lz_scr_settings(lv_obj_t *root)
{
    ...
    static const struct { const char *title; int first, count; } GROUPS[6] = {
        { "RADIO",        2, 2 }, { "CONNECTIVITY", 4, 3 },
        { "DISPLAY",      7, 3 }, { "TIME",        10, 3 },
        { "POWER",       13, 1 }, { "DEVICE",      14, 3 },
    };
    char bval[32];
    for(int g = 0; g < 6; g++) {
        ...
        for(int k = 0; k < GROUPS[g].count; k++) {
            ...
            switch(f) {
                ...
                case 5: value_chevron(row, lz_app_source_label(S.settings.app_source)); break;
                ...
                case 11: { char tb[12]; value_chevron(row, lz_fmt_now(tb, sizeof tb)); break; }
                case 12: value_chevron(row, S.settings.clock24 ? "24-hour" : "12-hour"); break;
                case 13: lz_toggle(row, S.settings.save, LZ_TOGGLE_ON); break;
                case 14: {
                ...
                case 15: value_chevron(row, ""); break;   /* Calibrate touch (NAV) */
                case 16: lz_toggle(row, S.settings.developer, LZ_TOGGLE_ON); break;
            }
            lz_nav_track(row, f);
        }
    }
